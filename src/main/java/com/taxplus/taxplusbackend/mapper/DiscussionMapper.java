...
public interface DiscussionMapper extends BaseMapper<Discussion> {
    ...
    @Select(value = "SELECT  d.*,  COALESCE(s.avatar, t.avatar) AS avatar, COALESCE(s.student_name, t.teacher_name) AS name FROM  discussion d LEFT JOIN students s ON d.commentator_name = s.student_id AND d.commentator_type = 'student' LEFT JOIN  teachers t ON d.commentator_name = t.teacher_id AND d.commentator_type= 'teacher';")
    ...
    @Select(value = "SELECT sd.*, COALESCE(s.avatar, t.avatar) AS avatar, COALESCE(s.student_name, t.teacher_name) AS name FROM sub_discussion sd LEFT JOIN students s ON sd.commentator_name = s.student_id AND sd.commentator_type = 'student' LEFT JOIN teachers t ON sd.commentator_name = t.teacher_id AND sd.commentator_type = 'teacher';")
